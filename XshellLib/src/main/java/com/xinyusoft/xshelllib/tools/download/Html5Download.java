package com.xinyusoft.xshelllib.tools.download;
...
public class Html5Download {
    ...
    public void checkH5(Context context, Activity activity) {
        ...
        String url = configInfo.get("html_url_list") + PreferenceUtil.getInstance().getFileUpdateTime()
    ...
    private void zipUpdateH5(String changezip) {
        ...
        if (null == changezip || "".equals(changezip)) {  //代表只删除了文件，没有其他文件下载
            time = PreferenceUtil.getInstance().getFileUpdateTime(); //就保存原先的
        } else {
    ...
    private void unZip(final String time, final String file, final File mfile) {
        try {
            ...
            PreferenceUtil.getInstance().setBackgroundUpdateFile(true);
