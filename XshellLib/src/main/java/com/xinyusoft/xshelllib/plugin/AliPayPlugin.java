...
public class AliPayPlugin extends CordovaPlugin {
	...
	public boolean execute(String action, JSONArray args,
		...
		try {
			...
			if ("aliPay".equals(action)) {
				...
				String orderInfo = getOrderInfo(jso);
	...
	public String getOrderInfo(JSONObject json) {
		...
		try {
			...
			// 商品金额
			 orderInfo += "&total_fee=" + "\"" + json.getString("total_fee")
			 + "\"";

			// 服务器异步通知页面路径
			orderInfo += "&notify_url=" + "\"" + json.getString("notify_url")
					+ "\"";
			...
			orderInfo += "&=\"" + json.getString("return_url") + "\"";
